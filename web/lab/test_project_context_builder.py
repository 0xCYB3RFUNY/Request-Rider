...
class ProjectContextBuilderTests(TestCase):
    def setUp(self):
        ...
        Finding.objects.create(
            ...
            severity="MEDIUM",
            confidence=80,
            target="https://app.example.test/api?token=finding-secret",
            fingerprint="knowledge-finding",
        )
        Finding.objects.create(
            project=self.other,
            title="Other finding",
            fingerprint="other-finding",
        )
    ...
    def test_summary_includes_project_osint_adjacency_and_dot(self):
        ...
        self.assertNotIn("Other finding", summary)
    ...
    def test_secret_model_has_no_plaintext_value_field(self):
        ...
        with self.assertRaises(IntegrityError), transaction.atomic():
            ProjectSecret.objects.create(
                ...
                key_name="invalid plaintext reference",
        ...
        with self.assertRaises(IntegrityError), transaction.atomic():
            ProjectSecret.objects.create(
                project=self.project,
                secret_type="api_key",
                key_name="invalid environment reference",
                value_ref="env:bad-name",
            )
...
class ProjectKnowledgeApiTests(TestCase):
    ...
    def test_project_api_accepts_bounded_tech_stack_and_notes(self):
        ...
        self.assertEqual(invalid_stack.status_code, 400)
        self.assertEqual(invalid_stack.json()["reason"], "INVALID_PROJECT_CONTEXT")

        invalid_notes = self.client.patch(
            f"/api/projects/{project['id']}",
            data=json.dumps({"notes": "x" * 20_001}),
            content_type="application/json",
        )
        self.assertEqual(invalid_notes.status_code, 400)
        self.assertEqual(invalid_notes.json()["reason"], "INVALID_PROJECT_CONTEXT")
