...
- `business_logic` — BOLA/IDOR differential, bounded race observation та
  bounded parallel request review;
...
- `Last-Byte Sync timing review` — real raw HTTP/1.1 loopback fixture із
  bounded final-byte hold; зовнішній target вимагає `LAST_BYTE_ALLOW_EXTERNAL=1`;
- `BOLA/IDOR differential evidence` — два authorized GET records і comparer;
- `Race-condition timing observation` — bounded parallel GET timing evidence;
...
browser-worker/.venv/bin/python tests/e2e/test_automation_ui.py -k template_store
```
