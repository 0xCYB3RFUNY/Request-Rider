...
Перед активним або passive-запитом engine викликає `ensureSourceIP`.
...
SOCKS5 route вимикається
    -> routeManager повертається до net.Dialer
        -> idle connections закриваються
            -> кожен новий запит окремо перевіряє IP
                -> нові запити йдуть напряму
```

Уже активний запит не обов'язково переривається миттєво. Новий маршрут
гарантовано використовується для нових з'єднань.
...
2. Apply проходить DOM validation, trim-ить address і відправляє PUT у Django.
   Gateway перевіряє method/JSON, Go route handler валідує SOCKS5 і перемикає
   dialer. `CloseIdleConnections` відділяє старі keep-alive від нових запитів.
3. Наступний Repeater/Intruder/OSINT/Scanner/Target exchange викликає спільний
   `dialContext`, тому route застосовується централізовано, а не копіюється в
   кожен інструмент.
...
6. Cancel route передає empty address і повертає Direct для нових dial.
   Вже активне з'єднання може завершитися старим transport; це не помилка
   перемикання і не підстава штучно змінювати його metadata.
