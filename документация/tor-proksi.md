...
`#route-check-url` задає check target.
...
`checkRoute` надсилає `{"url":"https://target/","timeout_ms":15000}`.
...
Timeout має бути між 1000 і 60000 ms.
...
Для Direct створюється `net.Dialer` timeout 10 s, keepalive 30 s.
Для SOCKS5 використовується `x/net/proxy.SOCKS5`.
Невалідний SOCKS5 configuration дає `INVALID_ROUTE`.
Після set engine закриває idle connections. Source IP не кешується: кожен
наступний outbound exchange повторно викликає
`https://check.torproject.org/api/ip` через актуальний transport.
Transport усіх активних інструментів використовує `dialContext`.
...
IP JSON очікує поле `ip`.
External result має status, status_code, ip/error, latency_ms.
...
UI route fields зберігаються у workspace localStorage state.
...
1. На відкритті вкладки UI читає route snapshot із engine; workspace може
   містити лише останнє введене поле, а не гарантує server configuration.
2. Apply trim-ить address і передає PUT. Route manager атомарно готує новий
   dialer, закриває idle connections і лише потім нові exchange бачать route.
...
6. Cancel route є explicit PUT з empty address. Він повертає Direct для
   наступних з'єднань, але вже активний request не обов'язково перериває.
7. Reload відновлює route fields/workspace text і повторно отримує actual
   engine snapshot; якщо engine restart-нувся, Direct є лише фактичним станом
   після нового GET, а не старим localStorage fallback.
