...
Browser-driven Target у локальному режимі використовує цей самий proxy
(`PASSIVE_PROXY_URL`, у Docker — `http://engine:8080`), тому його network
evidence отримує active route generation і `source_ip` без прямого обходу
engine. Django передає явний `proxy_mitm_ca` для ізольованого browser context;
upstream TLS перевіряє engine, а browser лише довіряє локальному MITM CA.
Pending event публікується перед fresh source-IP lookup; якщо browser closes
connection раніше, cancel/error update зберігає той самий ID.
...
5. SourceIP callback запускає один свіжий lookup через поточний route для кожного
   exchange; process-wide кеш відсутній, а pending event не чекає на зовнішній
   endpoint перед forwarding target request;
6. створюється Event із `source=proxy`, session, method, URL, host; під час
   Apply/Cancel route pending exchange отримує cancel/error update з тим самим
   ID, а не завершується success через старе з'єднання;
...
local proxy route; token не повертається browser API, а internal header
видаляється до forwarding upstream.
...
Rows keep two quick icons (Inspect, Repeater); the rest hides in the floating
`⋯` menu (`#ctx-menu`, trigger `data-ctx-traffic`): Intruder, Comparer
request/response, Decoder request/response, Tag/note, Copy cURL/JSON, Save
row, Send to Automation. Re-pressing `⋯` toggles the menu; its header shows
`#id · METHOD host` of the row, and the click never selects the row.
