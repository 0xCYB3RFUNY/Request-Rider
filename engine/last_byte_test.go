...
func TestLastByteSyncSendsFinalBodyByteAfterHold(t *testing.T) {
	...
	s := &server{lastByteJobs: make(map[uint64]*lastByteJob)}
...
func TestSendH2SinglePacketClearsEndStreamAndCoalescesFrames(t *testing.T) {
	conn := &recordingConn{}
	s := &server{}
	frameHeader := func(flags byte, streamID uint32, payload []byte) []byte {
	...
	first := frameHeader(0x01, 1, []byte("A"))
	second := frameHeader(0x01, 1, []byte("B"))
	if err := s.SendH2SinglePacket(conn, first, second); err != nil {
		t.Fatalf("SendH2SinglePacket() error = %v", err)
	}
	if len(conn.writes) != 1 {
		t.Fatalf("writes = %d, want 1", len(conn.writes))
	}
	payload := conn.writes[0]
	if len(payload) != len(first)+len(second) {
		t.Fatalf("payload length = %d, want %d", len(payload), len(first)+len(second))
	}
	if payload[4] != 0x00 || payload[4+len(first)] != 0x00 {
		t.Fatalf("END_STREAM flag was not cleared: %#v", payload)
	}
	if payload[3] != 0x00 || payload[3+len(first)] != 0x00 {
		t.Fatalf("frame types changed unexpectedly: %#v", payload)
	}
	if string(payload[9:10]) != "A" || string(payload[9+len(first):9+len(first)+1]) != "B" {
		t.Fatalf("payload bytes were not preserved: %#v", payload)
	}
}
