...
func TestRepeaterBurstRunsBoundedParallelRequests(t *testing.T) {
	...
	s := &server{bursts: make(map[uint64]*repeaterBurst)}
...
func TestRouteSwitchCancelsRepeaterBurst(t *testing.T) {
	started := make(chan struct{})
	...
	request := httptest.NewRequest(http.MethodPost, "/proxy/repeater-burst", strings.NewReader(`{"method":"GET","url":"`+provider.URL+`","iterations":4,"concurrency":1,"timeout_ms":2000}`))
	recorder := httptest.NewRecorder()
	s.repeaterBurstStart(recorder, request)
	var startedPayload map[string]interface{}
	if err := json.Unmarshal(recorder.Body.Bytes(), &startedPayload); err != nil {
		t.Fatal(err)
	}
	id := uint64(startedPayload["burst_id"].(float64))
	select {
	case <-started:
	case <-time.After(time.Second):
		t.Fatal("burst request did not start")
	...
	final := waitRepeaterBurst(t, s, id, "cancelled")
	if int(final["completed"].(float64)) != 0 {
		t.Fatalf("route-cancelled burst completed requests: %#v", final)
	}
}

func TestRepeaterBurstPreservesCompleteResponse(t *testing.T) {
	payload := strings.Repeat("x", 2*1024*1024+32)
	provider := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if result["body_truncated"] != false || len(result["body"].(string)) != len(payload) {
		t.Fatalf("complete response result = %#v", result)
	}
}
