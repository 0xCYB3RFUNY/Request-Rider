...
ensure_browser_worker() {
  ...
  if compgen -G "$playwright_path/firefox-*/firefox/firefox" >/dev/null 2>&1; then
    for firefox_binary in "$playwright_path"/firefox-*/firefox/firefox; do
      if [ -x "$firefox_binary" ]; then
        export FIREFOX_EXECUTABLE_PATH="$firefox_binary"
        echo "[run-engine] using cached Playwright Firefox runtime: $firefox_binary"
        return 0
      fi
    done
  fi
...
ensure_nuclei() {
  # Зібраний бінарник — runtime-артефакт проєкту, тому живе в корені (bin/),
  # а не в agent-workspace/tools/: там усе, що потрібне лише розробнику.
  local nuclei_bin="$ROOT_DIR/bin/nuclei"
  if [ -n "${RR_NUCLEI_BINARY:-}" ] && [ -x "$RR_NUCLEI_BINARY" ]; then
    echo "[run-engine] using RR_NUCLEI_BINARY=$RR_NUCLEI_BINARY"
    return 0
  fi
  if [ -x "$nuclei_bin" ]; then
    echo "[run-engine] using project Nuclei binary"
  elif command -v nuclei >/dev/null 2>&1; then
    nuclei_bin="$(command -v nuclei)"
    echo "[run-engine] using system Nuclei binary at $nuclei_bin"
  else
    echo "[run-engine] building pinned Nuclei binary (v3.11.1)"
    mkdir -p "$ROOT_DIR/bin"
    if ! GOBIN="$ROOT_DIR/bin" go install github.com/projectdiscovery/nuclei/v3/cmd/nuclei@v3.11.1; then
      echo "[run-engine] ERROR: Nuclei build failed; scanner integration will report RR_NUCLEI_BINARY."
      return 0
    fi
    nuclei_bin="$ROOT_DIR/bin/nuclei"
  fi
  export RR_NUCLEI_BINARY="${RR_NUCLEI_BINARY:-$nuclei_bin}"
  # Templates pack lives outside the repo on the operator machine; pick it up
  # automatically when present so the folder picker works out of the box.
  if [ -z "${RR_NUCLEI_TEMPLATES_DIR:-}" ] && [ -d "$HOME/Desktop/nuclei-templates" ]; then
    export RR_NUCLEI_TEMPLATES_DIR="$HOME/Desktop/nuclei-templates"
  fi
}
